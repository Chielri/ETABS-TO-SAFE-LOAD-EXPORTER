...
def build_label_cache(etabs_model):
    ...
    try:
        ...
        # Try common table names for area object connectivity/definitions
        for table_name in [
            ...
            if name_col is None or label_col is None:
                logger.debug("  Label cache: table '%s' missing columns, fields=%s", table_name, fields)
                continue
            ...
            logger.info("Cached %d label(s) from '%s'", len(cache), table_name)
            break  # First successful table is enough
    except Exception as e:
        logger.debug("  build_label_cache error: %s", e)
    return cache
    ...
    try:
        ...
        if ret[-1] == 0 and ret[4]:
            ...
            if name_col is not None and pat_col is not None:
                ...
                logger.info("Cached existing SAFE loads: %d slab(s) with loads", len(cache))
            else:
                logger.debug("  SAFE load table missing columns, fields=%s", fields)
        else:
            logger.debug("  SAFE load table empty or unavailable")
    except Exception as e:
        logger.debug("  build_safe_load_cache error: %s", e)
    return cache
...
def get_safe_slab_loads(safe_model, slab_name, safe_load_cache=None):
    ...
    try:
        ...
        logger.debug("  SAFE table read for existing loads failed: %s", e)
...
def bulk_delete_and_assign_loads(safe_model, deletions, assignments):
    ...
    try:
        ...
        logger.debug("  Bulk delete/assign failed: %s", e)
        return 0, 0
...
def run_export(progress_callback=None, etabs_pid=None, safe_pid=None):
    ...
    logger.info("Building caches from database tables...")
    ...
    logger.info("Caches ready: %d ETABS loads, %d labels, %d SAFE load entries",
                sum(len(v) for v in table_cache.values()) if table_cache else 0,
                len(label_cache),
                len(safe_load_cache))
    ...
    for idx, area_name in enumerate(selected_areas):
        ...
        if existing_safe_loads:
            ...
            logger.info("  SAFE slab '%s' has existing loads: %s", safe_slab_name, sorted(unique_patterns))
            deletions[safe_slab_name] = unique_patterns
        else:
            logger.info("  SAFE slab '%s' has no existing loads", safe_slab_name)

        # Ensure load patterns exist in SAFE (done here since it's cheap via COM)
        for load in loads:
    ...
    if assignments:
        logger.info("Writing %d load(s) to SAFE (%d deletions across %d slabs)...",
                     len(assignments), sum(len(v) for v in deletions.values()), len(deletions))
        num_deleted, num_assigned = bulk_delete_and_assign_loads(
            safe_model, deletions, assignments)
        logger.info("Bulk write complete: %d deleted, %d assigned", num_deleted, num_assigned)
        ...
        if num_assigned > 0:
            ...
            status = "FAILED"
            logger.error("Bulk write failed — 0 loads assigned")
    else:
        status = "No loads"

    # Build CSV rows for matched slabs
    for area_name, label, story, safe_slab_name, loads in slab_results:
        for load in loads:
            dir_name = DIR_NAMES.get(load["direction"], f"Dir-{load['direction']}")
            if status == "OK":
                logger.info("  Assigned: '%s' -> '%s', Pattern='%s', Value=%.4f",
                            area_name, safe_slab_name, load["load_pattern"], load["value"])
            csv_rows.append({
                "ETABS_UniqueName": area_name,
                "ETABS_Label": label,
                "Level": story,
